...
| compare shows a change in what RAN, not only in findings | [fixed] | #28: `skipped` and `quiet` were ignored, so a change making a check start or stop running compared as no change — the verifier blind to "found nothing" vs "did not look". Now `- [skip]` / `+ [skip]` and `- [quiet]` / `+ [quiet]` lines, with a control that identical reports carrying skips stay silent |
